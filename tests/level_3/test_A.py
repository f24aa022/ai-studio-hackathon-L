    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for video_id, name in expected_videos.items():
                locator = page.locator(f'iframe[src*="{video_id}"]')
                count = locator.count()
                ...
                if count > 0:
                    ...
                    if (
                        "youtube.com/embed" in src
                        or "youtube-nocookie.com/embed" in src
                    ):
                        print(f"  {name}: ✅ 埋め込みあり (src={src})")
                        results[name] = True
                    else:
                        print(f"  {name}: ❌ iframeはあるがYouTube埋め込み形式ではありません")
                        results[name] = False
                else:
                    print(f"  {name}: ❌ 埋め込みなし")
                    results[name] = False
            ...
            print("❌ エラー: iframeが見つかりません（動画が埋め込まれていない可能性）")
            browser.close()
            return False
...
def main():
    ...
    print("=" * 60)
    print("※ ポート3001でアプリケーションが起動している必要があります")
    print("=" * 60)
    print()
    ...
    if result:
        ...
        print("例: <iframe src=\"https://www.youtube.com/embed/VIDEO_ID\"></iframe>")
        sys.exit(1)
